    ...
    <script>
        ...
        function loadHistory(name) {
            db.collection("txs").where("user", "==", name).orderBy("time", "desc").onSnapshot(s => {
                ...
                document.getElementById('history-list').innerHTML = h || "<p class='text-center opacity-20 text-xs'>No Records</p>";
            });
        }
        ...
        function loadAdmin() {
            ...
            db.collection("txs").where("status","==","Pending").onSnapshot(s => {
                let h = ""; s.forEach(doc => {
                    const d = doc.data();
                    h += `<div class="glass p-4"><p class="text-[10px]">${d.user} (${d.type})</p><h2 class="text-xl font-black">₨ ${d.amount}</h2><button onclick="approveTx('${doc.id}','${d.user}',${d.amount})" class="w-full bg-green-600 py-2 mt-2 rounded-lg font-bold text-[9px]">APPROVE</button></div>`;
                });
                document.getElementById('adm-tx-list').innerHTML = h || "All Clean!";
            });
        }
        ...
        function sendMsg() { 
            ...
            db.collection("chats").add({ user: auth.currentUser.displayName, msg: input.value, sender: 'user', time: Date.now() });
            input.value = "";
        }
        function loadChat(name) {
            db.collection("chats").where("user","==",name).orderBy("time","asc").onSnapshot(s => {
                ...
                const box = document.getElementById('chat-msgs'); box.innerHTML = h; box.scrollTop = box.scrollHeight;
            });
        }
        function startFakeTicker() {
            setInterval(() => {
                const names = ["@Ali_78", "@Sara_99", "@Khan_007"];
                const n = names[Math.floor(Math.random()*names.length)];
                document.getElementById('ticker-content').innerText = `⚡ ${n} نے ابھی رقم نکلوائی • سپورٹ ٹیم ایکٹو ہے • ` + document.getElementById('ticker-content').innerText.substring(0, 50);
            }, 8000);
        }
